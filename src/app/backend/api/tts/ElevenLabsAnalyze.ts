...
export async function generateSpeech(
  userPrompt: string,
): Promise<{ buffer: Buffer; action: string }> {
  try {
    // Read chat history
    let chatHistory: ChatMessage[] = [];
    try {
    ...
    let actionType = "FOLLOW_UP"; // Default action
    ...
    try {
      if (
        ...
        // Get the action from the LLM response
        actionType = assistantMessage.action;
      ...
      if (assistantMessageRaw) {
        ...
        // Try to extract action with regex as well
        const actionMatch = assistantMessageRaw.match(
          /"action"\s*:\s*"([^"]*)"/,
        );
        if (actionMatch && actionMatch[1]) {
          actionType = actionMatch[1] as "FOLLOW_UP" | "GUIDANCE" | "READY";
        }
      }
    }
    ...
    // Return both the buffer and the action type
    return {
